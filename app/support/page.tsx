...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: d.support.meta.title, description: d.support.meta.description, path: '/support' });
}
...
export default async function SupportPage() {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <section className="relative overflow-hidden bg-black pb-24 text-white"><div className="brand-grid absolute inset-0 opacity-45" /><SiteHeader /><div className="relative mx-auto max-w-7xl px-5 pt-14 lg:px-8 lg:pt-20"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#52c6ff]">{s.eyebrow}</p><h1 className="mt-6 max-w-5xl text-5xl font-light leading-[1] tracking-[-.05em] sm:text-6xl lg:text-7xl">{s.title}</h1><p className="mt-7 max-w-2xl text-lg leading-8 text-white/60">{s.intro}</p><a href={CONTACT_PHONE_HREF} className="mt-9 inline-flex rounded-full bg-[#4556f5] px-7 py-4 text-sm font-bold text-white">{CONTACT_PHONE_DISPLAY}</a></div></section>
