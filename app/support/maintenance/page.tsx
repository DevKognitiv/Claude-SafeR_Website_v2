...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: d.support.maintenance.meta.title, description: d.support.maintenance.meta.description, path: '/support/maintenance', slot: 'maintenance' });
}
...
export default async function MaintenancePage() {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <section className="bg-black text-white"><SiteHeader /><div className="mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-4 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:px-8"><div><Breadcrumbs items={[{ label: d.nav.support, href: '/support' }, { label: m.breadcrumb }]} /><p className="mt-10 text-xs font-bold uppercase tracking-[.16em] text-[#52c6ff]">{m.eyebrow}</p><h1 className="mt-5 text-5xl font-light leading-[1] tracking-[-.05em] sm:text-6xl">{m.title}</h1><p className="mt-6 max-w-xl text-lg leading-8 text-white/60">{m.intro}</p></div><div className="media-grade relative min-h-[460px] overflow-hidden rounded-[34px]"><SmartImage slot="maintenance" alt={d.images.maintenance} loading="eager" className="absolute inset-0 h-full w-full object-cover" /></div></div></section>
