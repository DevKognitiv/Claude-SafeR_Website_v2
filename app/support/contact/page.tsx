...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: d.support.contact.meta.title, description: d.support.contact.meta.description, path: '/support/contact' });
}
...
export default async function ContactPage() {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <section className="bg-black text-white"><SiteHeader /><div className="mx-auto max-w-7xl px-5 pb-20 pt-4 lg:px-8"><Breadcrumbs items={[{ label: d.nav.support, href: '/support' }, { label: c.eyebrow }]} /><p className="mt-10 text-xs font-bold uppercase tracking-[.16em] text-[#52c6ff]">{c.eyebrow}</p><h1 className="mt-5 max-w-4xl text-5xl font-light leading-[1] tracking-[-.05em] sm:text-6xl">{c.title}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-white/60">{c.intro}</p></div></section>
