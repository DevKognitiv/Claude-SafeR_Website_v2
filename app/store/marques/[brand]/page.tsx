...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: `${brand.name} · ${d.store.brands.brandPage.metaSuffix}`, description: `${fmt(d.store.brands.productsCount, { count: brand.count })} · ${d.store.brands.intro}`, path: `/store/marques/${id}`, slot: 'store' });
}
...
export default async function BrandPage({ params }: { params: Params }) {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <ItemListJsonLd name={`${brand.name} · ${b.brandPage.metaSuffix}`} path={`/store/marques/${id}`} items={items} />
