...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: `${title} · ${d.store.categories[category].title}`, description: d.store.categories[category].description, path: `/store/${category}/${subcategory}`, slot: 'store' });
}
...
export default async function StoreSubcategoryPage({ params }: { params: Params }) {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <ItemListJsonLd name={`${title} · ${cat.title}`} description={cat.description} path={`/store/${category}/${subcategory}`} items={items} />
    ...
    <section className="px-5 py-16 lg:px-8 lg:py-20"><div className="mx-auto max-w-7xl"><StoreCatalog items={items} brands={brandNames()} lock={{ category, subcategory }} /></div></section>
