...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: d.store.meta.title, description: d.store.meta.description, path: '/store', slot: 'store' });
}

export default async function StorePage() {
  const { d, locale } = await getI18n();
  const s = d.store;
  const items = storeItems(locale);
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    ...
    <section className="px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl"><div className="mb-12 grid gap-6 lg:grid-cols-[1fr_.8fr] lg:items-end"><div><p className="eyebrow">{s.catalog.eyebrow}</p><h2 className="section-title mt-5">{s.catalog.title1}<br />{s.catalog.title2}</h2></div><p className="max-w-xl leading-7 text-black/55 lg:ml-auto">{s.catalog.intro}</p></div><StoreCatalog items={items} brands={brandNames()} showCategoryFacet /></div></section>
