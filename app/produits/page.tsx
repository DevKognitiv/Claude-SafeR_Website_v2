...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return pageMetadata(d, locale, { title: d.solutions.showcase.meta.title, description: d.solutions.showcase.meta.description, path: '/produits', slot: 'showroom' });
}
    ...
    <section className="px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-14 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {productFamilies.map((product, index) => { const copy = sc.families[product.id]; return <article key={product.id} className={`flex min-h-[360px] flex-col rounded-[30px] border p-7 ${index === 0 ? 'border-transparent bg-[#4556f5] text-white' : 'border-black/8 bg-white'}`}>
          ...
          <div className="mt-auto pt-8"><Link href={`/store?q=${encodeURIComponent(product.storeSku)}`} className={`inline-flex rounded-full px-5 py-3 text-xs font-bold ${index === 0 ? 'bg-white text-black' : 'bg-black text-white'}`}>{d.solutions.detail.viewInStore} →</Link></div>
        </article>; })}
      </div>
    </div></section>
    <TrustStrip />
