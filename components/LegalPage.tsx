...
export default async function LegalPage({ title, sections }: { title: string; sections: string[][] }) {
  ...
  return <main id="contenu" className="theme-page bg-[#f3f5fb] text-black">
    <section className="bg-black text-white"><SiteHeader /><div className="mx-auto max-w-7xl px-5 pb-16 pt-4 lg:px-8"><Breadcrumbs items={[{ label: title }]} /><h1 className="mt-10 max-w-4xl text-5xl font-light leading-[1] tracking-[-.05em] sm:text-6xl">{title}</h1><p className="mt-5 text-sm text-white/50">{d.legal.lastUpdated}</p></div></section>
